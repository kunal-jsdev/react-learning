...
import todoQueryOptions from './queries/queryOptions'
...
const Page = () => {
    ...
    const {data} = useQuery(todoQueryOptions());
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchTerm.trim()) result = result.filter(i => i.todo.toLowerCase().includes(searchTerm.toLowerCase()));
        return result;
                ...
                <div className = 'mt-6 w-130 overflow-auto h-136 flex flex-col items-center'>
                    {filteredData?.map((item) => (
                        <div key={item.id} className={` border-b border-violet-500 shadow-sm px-3 py-2 mb-3 w-130 flex justify-start`}>
                            ...
                            <div className='font-medium font-kanit text-xl'>{item.completed ? <s className='text-gray-400'>{`${item.todo} #${item.id}`}</s> : `${item.todo} #${item.id}`}</div>
                        </div>
                    ))}
                </div>
